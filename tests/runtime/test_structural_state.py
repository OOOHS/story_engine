...
def test_rendering_filters_actor_director_fields_and_hidden_change_ledgers():
    ...
    player_pov = {
        ...
        "visible_actor_states": {
            "甲": {"location": "大厅"},
            "乙": {"location": "大厅", "stance": "seated"},
        },
        "visible_world": {
            "大厅": {},
            "铜门": {"appearance": "青铜门", "container_open": True},
        },
    }
            ...
            "state_updates": {
                ...
                "actor_states": {
                    "乙": {
                        "stance": "seated",
                        "bias": "甲",
                        "dramatic_motive": "诱导冲突",
                        "capabilities": ["deception"],
                        "public_state_fields": ["dramatic_motive"],
                    }
                },
            },
            "actor_movements": [
    ...
    assert visible["state_updates"]["actor_states"] == {
        "乙": {"stance": "seated"}
    }
    assert visible["state_updates"]["world_objects"] == {
        "铜门": {"appearance": "青铜门", "container_open": True}
    }
    assert visible["actor_movements"] == []
    assert visible["object_state_changes"] == [
        {
            "object_id": "铜门",
            "paths": ["open"],
            "visibility": "local",
    ...
    assert visible["state_updates"]["scene"] == {"alarm": "ringing"}
    assert visible["scene_state_changes"] == [
        {
            "path": "alarm",
            "value": "ringing",
            "visibility": "public",
    ...
    assert visible["storylet_hits"] == []
    assert visible["causal_plot_rules"] == []
    assert visible["simulation_notes"] == []
    assert RenderingSystem._public_social(
        {
            "viewer": "甲",
            "visible_relations": [
                {
                    "actor": "乙",
                    "bias": "甲",
                    "framing_style": "manipulative",
                    "territorial": True,
                    "toward_viewer_states": ["wary"],
                    "relationship_bits": ["acquainted"],
                }
            ],
    ...
    assert RenderingSystem._public_timeline(
        {
            "day_phase": "night",
            "phase_turn": 7,
            "phase_transition": {"from": "evening", "to": "night"},
            "due_commitments": [{"participants": ["乙"], "secret": "hidden"}],
            "transition_pressure": {"carrier_states": {"乙": {"bias": "甲"}}},
            "last_missed_commitment": {
                "commitment_id": "meeting",
                "title": "秘密会议",
                "location": "密室",
                "note": "甲错过了会议。",
                "missing_participants": ["甲"],
                "private_rule": "hidden",
            },
        },
        {
            "viewer": "甲",
            "public_scene": {"flags": {"day_phase": "night"}},
        },
    ) == {
        "day_phase": "night",
        "phase_transition": {"from": "evening", "to": "night"},
