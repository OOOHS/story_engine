...
class SentimentDynamics:
    ...
    def apply_agreement_transitions(
        self,
        *,
        sentiment_states: Dict[str, Any],
        relationship_book: Any,
        ...
        """Derive participant-local reactions from authoritative performance facts."""
        applied: List[Dict[str, Any]] = []
        mapping = {
            "fulfilled": ("grateful", 0.65),
            "breached": ("betrayed", 0.85),
            "cancelled": ("hurt", 0.5),
        }
        for transition in transitions or []:
            if not isinstance(transition, dict):
                continue
            status = str(transition.get("performance_status", "")).strip()
            if status not in mapping:
                continue
            agreement_id = str(
                transition.get("agreement_id")
                or transition.get("contract_id")
                or ""
            ).strip()
            record = agreement_book.agreements.get(agreement_id) if agreement_book else None
            if record is None:
                continue
            kind, magnitude = mapping[status]
            definition = self.definitions.get(kind)
            if definition is None:
                continue
            responsible = {
                str(link.get("actor", "")).strip()
                for link in record.performance_obligations
                if isinstance(link, dict)
                and str(link.get("resolved_status", "")).strip() == status
                and str(link.get("actor", "")).strip()
            }
            for source in sorted(responsible):
                for affected in record.parties:
                    if affected == source or affected not in sentiment_states:
                        continue
                    reason = (
                        f"Agreement {agreement_id} performance became {status}"
                    )
                    sentiment = sentiment_states[affected].upsert(
                        toward=source,
                        kind=kind,
                        magnitude=magnitude,
                        valence=definition.valence,
                        ...
                        policy_weights=definition.policy_weights,
                        source_event=(
                            f"agreement_performance_resolution:{agreement_id}:{status}"
                        ),
                    )
                    track_changes = {}
                    if relationship_book is not None:
                        ...
                        after = relationship_book.apply_delta(
                            ...
                            provenance={
                                "source_kind": "sentiment",
                                "source_ref": f"{affected}:{sentiment.sentiment_id}",
                                "source_event": sentiment.source_event,
                            },
                            **{
                                f"{track_id}_delta": delta * magnitude
                                for track_id, delta in definition.track_effects.items()
                        ...
                        track_changes = {
                            track_id: after.get(track_id, 0.0)
                            - before.get(track_id, 0.0)
                            for track_id in definition.track_effects
                        }
                    applied.append(
                        {
